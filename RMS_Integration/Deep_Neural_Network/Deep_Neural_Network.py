...
def load_dataset(trainsize, testsize, filename):
...
def initialize_calibration(mini_batch_size):

    Kx = np.random.randn(mini_batch_size) * 0.01
    Ky = np.random.randn(mini_batch_size) * 0.01
    Bx = np.random.randn(mini_batch_size) * 0.01
    by = np.random.randn(mini_batch_size) * 0.01
    bx = np.random.randn(mini_batch_size) * 0.01
    
    return (Kx, Ky, Bx, by, bx)
...
def update_beta(Kx, Ky, Bx, by, bx, learning_rate, cost):
    
    Kx = Kx - learning_rate * cost
    Ky = Ky - learning_rate * cost
    Bx = Bx - learning_rate * cost
    by = by - learning_rate * cost
    bx = bx - learning_rate * cost
...
def backward_propagation_deep(AL, Y, caches, lamdb, cost_method):
    gradients = {}
    L = len(caches)
    m = AL.shape[0]
    ...
    if cost_method == "MSLE":
        dAL = (2*np.log(AL + 1) - 2*np.log(Y + 1))/(AL*m + m)
    
    elif cost_method == "MSE":
        dAL = AL - Y #+ lambd/m
...
def NN_model(X, Y, layers_dims, learning_rate, mini_batch_size, beta, num_epochs, lamdb, cost_method, calibration, optimizer, 
    ...
    if calibration is not None:
        Beta_P = []
        Kx, Ky, Bx, by, bx = initialize_calibration(mini_batch_size)
    ...
    for i in range(num_epochs):
        ...
        for minibatch in minibatches:
            ...
            if calibration is not None:
                Beta = np.array([Kx ** 2, Ky ** 2, 2 * Bx * Ky**2, 2 * Kx * bx, 2 * Ky * by])
                minibatch_X2 = np.multiply(minibatch_X, Beta)
            ...
            gradients = backward_propagation_deep(AL, minibatch_Y, caches, lamdb, cost_method)
            ...
            if optimizer == "gd":
                ...
                Kx, Ky, Bx, by, bx = update_beta(Kx, Ky, Bx, by, bx, learning_rate, cost)
                Beta_P.append(Kx, Ky, Bx, by, bx)
    ...
    return parameters, costs, cost_i
